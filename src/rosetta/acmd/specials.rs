...
unsafe extern "C" fn rosa_boom(fighter: &mut L2CAgentBase) {
	...
	if macros::is_excute(fighter) && !VariableModule::is_flag((fighter.module_accessor) as *mut _, FIGHTER_ROSETTA_INSTANCE_WORK_ID_FLAG_IS_TICO_DEAD){
...
unsafe extern "C" fn luma_boom(fighter: &mut L2CAgentBase) {
	...
	if ![*WEAPON_ROSETTA_TICO_STATUS_KIND_DEAD, *WEAPON_ROSETTA_TICO_STATUS_KIND_NONE].contains(&status_kind) && !VariableModule::is_flag((boma) as *mut _, FIGHTER_ROSETTA_INSTANCE_WORK_ID_FLAG_IS_TICO_DEAD) {
