...
pub fn install() {
	...
	param_config::update_float_2(-*FIGHTER_KIND_ROSETTA, get_marked_costumes("rosetta","rosetta").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_tico"), smash::hash40("hp"), 30.0));
	param_config::update_float_2(-*FIGHTER_KIND_ROSETTA, get_marked_costumes("rosetta","rosetta").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_tico"), smash::hash40("follow_damage_air_brake"), 0.075));
	param_config::update_float_2(-*FIGHTER_KIND_ROSETTA, get_marked_costumes("rosetta","rosetta").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_tico"), smash::hash40("free_damage_air_brake"), 0.07));
