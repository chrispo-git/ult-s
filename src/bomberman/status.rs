...
unsafe extern "C" fn main_neutralb(fighter: &mut L2CFighterCommon) -> L2CValue {
        ...
        if ![hash40("special_n"), hash40("special_n_hold"), hash40("special_n_shoot"), hash40("special_air_n"), hash40("special_air_n_hold"), hash40("special_air_n_shoot")].contains(&motion_kind){
            ...
            if [hash40("special_n"), hash40("special_air_n")].contains(&motion_kind) {
                ...
                if is_end {
                    if VariableModule::get_int((fighter.module_accessor) as *mut _, FIGHTER_BOMBERMAN_INSTANCE_WORK_ID_INT_NEUTRALB_CHARGE) >= NEUTRALB_MAX {
                        VariableModule::set_float((fighter.module_accessor) as *mut _, MIN_DISTANCE + ((MAX_DISTANCE-MIN_DISTANCE))*((VariableModule::get_int((fighter.module_accessor) as *mut _, FIGHTER_BOMBERMAN_INSTANCE_WORK_ID_INT_NEUTRALB_CHARGE) as f32)/(NEUTRALB_MAX as f32)), FIGHTER_BOMBERMAN_INSTANCE_WORK_ID_FLOAT_NEUTRALB_DIST);
                        if is_ground {
            ...
            if [hash40("special_n_hold"), hash40("special_air_n_hold")].contains(&motion_kind) {
                ...
                VariableModule::inc_int((fighter.module_accessor) as *mut _, FIGHTER_BOMBERMAN_INSTANCE_WORK_ID_INT_NEUTRALB_CHARGE);
                ...
                if VariableModule::get_int((fighter.module_accessor) as *mut _, FIGHTER_BOMBERMAN_INSTANCE_WORK_ID_INT_NEUTRALB_CHARGE) >= NEUTRALB_MAX || jc || sc  {
                    if is_ground {
            ...
            if [hash40("special_n_shoot"), hash40("special_air_n_shoot")].contains(&motion_kind) {
                ...
                smash::app::lua_bind::FighterKineticEnergyMotion::set_speed_mul(fighter_kinetic_energy_motion, VariableModule::get_float((fighter.module_accessor) as *mut _, FIGHTER_BOMBERMAN_INSTANCE_WORK_ID_FLOAT_NEUTRALB_DIST));
				if !is_ground {
                    if frame < 23.0  {
