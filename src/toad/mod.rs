...
pub fn install() {
    ...
    param_config::update_float_2(*FIGHTER_KIND_MURABITO, get_marked_costumes("murabito","toad").into_iter().map(|x| x as i32).collect(), (smash::hash40("dash_speed"), 0, 2.35));
    ...
    param_config::update_float_2(*FIGHTER_KIND_MURABITO, get_marked_costumes("murabito","toad").into_iter().map(|x| x as i32).collect(), (smash::hash40("weight"), 0, 92.0));
