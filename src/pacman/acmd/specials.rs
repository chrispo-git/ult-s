...
unsafe extern "C" fn pac_empty_downb(fighter: &mut L2CAgentBase) {
		...
		if macros::is_excute(fighter) {
            SET_SPEED_EX(fighter, 0.0, 0.3, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
		}
		frame(fighter.lua_state_agent, 14.0);
		if macros::is_excute(fighter) {
            SET_SPEED_EX(fighter, 0.0, -2.8, *KINETIC_ENERGY_RESERVE_ATTRIBUTE_MAIN);
...
unsafe extern "C" fn pac_empty_downb_eff(fighter: &mut L2CAgentBase) {
    ...
    frame(fighter.lua_state_agent, 14.0);
    if macros::is_excute(fighter) {
        macros::EFFECT_FOLLOW(fighter, Hash40::new("sys_machstamp"), Hash40::new("hip"), 0, 0.0, 0, 0, 0, 0, 0.5, true);
    }
    frame(fighter.lua_state_agent, 45.0);
    if macros::is_excute(fighter) {
