...
pub fn install() {
    ...
    .acmd("game_attackairlw", pac_dair, Priority::Low)    
    .acmd("effect_attackairlw", pac_dair_eff, Priority::Low)    
    .acmd("sound_attackairlw", pac_dair_snd, Priority::Low)    
    .install();
}

unsafe extern "C" fn pac_uair(fighter: &mut L2CAgentBase) {
...
unsafe extern "C" fn pac_dair(fighter: &mut L2CAgentBase) {
        let lua_state = fighter.lua_state_agent;
...
unsafe extern "C" fn pac_dair_eff(fighter: &mut L2CAgentBase) {
        let lua_state = fighter.lua_state_agent;
...
unsafe extern "C" fn pac_dair_snd(fighter: &mut L2CAgentBase) {
    frame(fighter.lua_state_agent, 4.0);
    if macros::is_excute(fighter) {
