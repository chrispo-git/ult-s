...
pub fn install() {
	...
	param_config::update_int_2(-*WEAPON_KIND_ROBOT_BEAM, get_marked_costumes("robot","robot").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_beam"), smash::hash40("life"), 25));
	param_config::update_int_2(-*WEAPON_KIND_ROBOT_BEAM, get_marked_costumes("robot","robot").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_beam"), smash::hash40("strong_life"), 105));
	param_config::update_float_2(-*WEAPON_KIND_ROBOT_BEAM, get_marked_costumes("robot","robot").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_beam"), smash::hash40("strong_speed"), 1.6));
