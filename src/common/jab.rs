    ...
    unsafe { 
		...
		if (config::get().attacks.jab_cancel == 2 && !is_gamemode("fgmode".to_string()) {
			if crate::get_state!(ENTRY_ID, JabState).has_enable_100_on {
				...
				if crate::get_state!(ENTRY_ID, JabState).has_enable_combo_on {
				...
				if crate::get_state!(ENTRY_ID, JabState).has_enable_no_hit_combo_on {
					WorkModule::set_flag(boma(fighter), true, *FIGHTER_STATUS_ATTACK_FLAG_ENABLE_COMBO);
				};
			return;
		}
		...
		if crate::is_in!(status_kind, *FIGHTER_STATUS_KIND_ATTACK_100, *FIGHTER_STATUS_KIND_ATTACK, *FIGHTER_DEMON_STATUS_KIND_ATTACK_COMBO) {
		...
		&& (crate::get_state!(ENTRY_ID, JabState).has_enable_combo_on || crate::get_state!(ENTRY_ID, JabState).has_enable_100_on){
				if  (AttackModule::is_infliction_status(boma(fighter), *COLLISION_KIND_MASK_HIT) || (config::get().attacks.jab_cancel == 1 && AttackModule::is_infliction_status(boma(fighter), *COLLISION_KIND_MASK_ALL))) {
