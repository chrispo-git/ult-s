...
static FIGHTERS_F10: Lazy<Vec<i32>> = Lazy::new(|| {
    vec![
...
static FIGHTERS_F11: Lazy<Vec<i32>> = Lazy::new(|| {
    vec![*FIGHTER_KIND_MIIGUNNER]
});

static FIGHTERS_F12: Lazy<Vec<i32>> = Lazy::new(|| {
    vec![
...
static FIGHTERS_F14: Lazy<Vec<i32>> = Lazy::new(|| {
    vec![
        *FIGHTER_KIND_BAYONETTA, *FIGHTER_KIND_MARTH, *FIGHTER_KIND_NESS, *FIGHTER_KIND_DEDEDE, 
        *FIGHTER_KIND_BRAVE, *FIGHTER_KIND_DEMON, *FIGHTER_KIND_LUCINA, *FIGHTER_KIND_SHIZUE
    ]
});

static FIGHTERS_F16: Lazy<Vec<i32>> = Lazy::new(|| {
    vec![
...
unsafe fn dacus(fighter : &mut L2CFighterCommon, status_kind : i32, ENTRY_ID : usize) {
	...
	let frame_max = match fighter_kind {
		n if FIGHTERS_F6.contains(&n) => 6,
		n if FIGHTERS_F10.contains(&n) => 10,
		n if FIGHTERS_F11.contains(&n) => 11,
		n if FIGHTERS_F12.contains(&n) => 12,
		n if FIGHTERS_F14.contains(&n) => 14,
		n if FIGHTERS_F16.contains(&n) => 16,
		_ => 8,
	};
