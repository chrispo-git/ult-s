...
unsafe extern "C" fn is_throw_stick(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let attack_lw3_stick_x = WorkModule::get_param_float(fighter.boma(), hash40("common"), hash40("attack_lw3_stick_x"));
    let attack_hi4_stick_y = WorkModule::get_param_float(fighter.boma(), hash40("common"), hash40("attack_hi4_stick_y"));
    let attack_lw4_stick_y = WorkModule::get_param_float(fighter.boma(), hash40("common"), hash40("attack_lw4_stick_y"));
    if stick_x > attack_lw3_stick_x {
        out["f"] = true.into();
    } else if stick_x < -attack_lw3_stick_x {
        out["b"] = true.into();
    }
    if stick_y > attack_hi4_stick_y {
        out["hi"] = true.into();
    } else if stick_y < attack_lw4_stick_y {
        out["lw"] = true.into();
    }
    out
}
