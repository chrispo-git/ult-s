...
unsafe fn roy_ftilt(fighter: &mut L2CAgentBase) {
    ...
    acmd!(lua_state, {
		...
		if(is_excute){
			...
			ATTACK(ID=1, Part=0, Bone=hash40("top"), Damage=9.0, Angle=90, KBG=80, FKB=0, BKB=40, Size=6.2, X=0.0, Y=8.0, Z=23.5, X2=LUA_VOID, Y2=LUA_VOID, Z2=LUA_VOID, Hitlag=0.85, SDI=1.0, Clang_Rebound=ATTACK_SETOFF_KIND_ON, FacingRestrict=ATTACK_LR_CHECK_F, SetWeight=false, ShieldDamage=0, Trip=0.0, Rehit=0, Reflectable=false, Absorbable=false, Flinchless=false, DisableHitlag=false, Direct_Hitbox=true, Ground_or_Air=COLLISION_SITUATION_MASK_GA, Hitbits=COLLISION_CATEGORY_MASK_ALL, CollisionPart=COLLISION_PART_MASK_ALL, FriendlyFire=false, Effect=hash40("collision_attr_fire"), SFXLevel=ATTACK_SOUND_LEVEL_L, SFXType=COLLISION_SOUND_ATTR_FIRE, Type=ATTACK_REGION_SWORD)
		}
		wait(Frames=3)
		FT_MOTION_RATE(FSM=1.53)
		if(is_excute){
			AttackModule::clear_all()
		}
		frame(Frame=27)
