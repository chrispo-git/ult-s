...
pub fn install() {
	...
	param_config::update_float_2(-*WEAPON_KIND_ZELDA_DEIN, get_marked_costumes("zelda","zelda").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_dein"), smash::hash40("size_max"), 4.2));
	param_config::update_float_2(-*WEAPON_KIND_ZELDA_DEIN, get_marked_costumes("zelda","zelda").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_dein"), smash::hash40("bang_time"), 2.0));
