...
pub fn install() {
	...
	param_config::update_float_2(-*FIGHTER_KIND_WIIFIT, get_marked_costumes("wiifit","wiifit").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_sunbullet"), smash::hash40("speed_min"), 4.0));
	param_config::update_float_2(-*FIGHTER_KIND_WIIFIT, get_marked_costumes("wiifit","wiifit").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_sunbullet"), smash::hash40("speed_max"), 3.5));
	param_config::update_float_2(-*FIGHTER_KIND_WIIFIT, get_marked_costumes("wiifit","wiifit").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_sunbullet"), smash::hash40("scale_min"), 0.1));
	param_config::update_float_2(-*FIGHTER_KIND_WIIFIT, get_marked_costumes("wiifit","wiifit").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_sunbullet"), smash::hash40("scale_max"), 0.5));
	param_config::update_int_2(-*FIGHTER_KIND_WIIFIT, get_marked_costumes("wiifit","wiifit").into_iter().map(|x| x as i32).collect(), (smash::hash40("param_sunbullet"), smash::hash40("life"), 30));
