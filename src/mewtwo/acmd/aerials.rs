...
pub fn install() {
    Agent::new("mewtwo")
    .acmd("game_attackairn", m2_nair, Priority::Low) 
...
unsafe extern "C" fn m2_nair(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn m2_bair_eff(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
		...
		if macros::is_excute(fighter) {
			macros::EFFECT_FOLLOW_FLIP(fighter, Hash40::new("sys_smash_flash"), Hash40::new("sys_smash_flash"), Hash40::new("handr"), 0, 0, 0, 0, 0, 0, 0.8, true, *EF_FLIP_YZ);
		}
		frame(fighter.lua_state_agent, 13.0);
		if macros::is_excute(fighter) {
...
unsafe extern "C" fn m2_bair_snd(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
		...
		if macros::is_excute(fighter) {
			macros::PLAY_STATUS(fighter, Hash40::new("se_mewtwo_smash_s01"));
