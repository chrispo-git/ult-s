        ...
        let boma = smash::app::sv_system::battle_object_module_accessor(fighter.lua_state_agent); 
		{
            ...
            if  [*FIGHTER_MURABITO_STATUS_KIND_SPECIAL_N_SEARCH, *FIGHTER_STATUS_KIND_SPECIAL_N, *FIGHTER_MURABITO_STATUS_KIND_SPECIAL_N_POCKET].contains(&status_kind) {
                if frame >= cancel_frame {
                    VariableModule::set_flag((boma) as *mut _, false, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_CHANGE_FRAME);
            ...
            if [hash40("special_air_n")].contains(&MotionModule::motion_kind(boma)) {
                if !VariableModule::is_flag((boma) as *mut _, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_DO_BOUNCE) {
                    if AttackModule::is_infliction_status(boma, *COLLISION_KIND_MASK_ALL) && !AttackModule::is_infliction(boma, *COLLISION_KIND_MASK_ALL){
                        ...
                        VariableModule::set_flag((boma) as *mut _, true, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_DO_BOUNCE);
                    };
                }
            ...
            if situation_kind != *SITUATION_KIND_AIR {
                VariableModule::set_flag((boma) as *mut _, false, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_DO_BOUNCE);
            }
            ...
            if  [*FIGHTER_MURABITO_STATUS_KIND_SPECIAL_N_SEARCH, *FIGHTER_STATUS_KIND_SPECIAL_N, *FIGHTER_MURABITO_STATUS_KIND_SPECIAL_N_POCKET].contains(&status_kind) {
                ...
                if StatusModule::is_situation_changed(boma) {
                    ...
                    if VariableModule::is_flag((boma) as *mut _, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_HAS_BEEN_AIR) && situation_kind == SITUATION_KIND_GROUND {
                        StatusModule::change_status_request_from_script(fighter.module_accessor, *FIGHTER_STATUS_KIND_LANDING, false);
                        VariableModule::set_flag((boma) as *mut _, false, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_HAS_BEEN_AIR);
                    }
                    if frame < 2.0 && situation_kind == *SITUATION_KIND_GROUND && ![hash40("special_n3"), hash40("special_n2_fail"), hash40("special_n2")].contains(&motion_kind) && !VariableModule::is_flag((boma) as *mut _, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_CHANGE_FRAME){
                        if ArticleModule::is_exist(fighter.module_accessor, *FIGHTER_MURABITO_GENERATE_ARTICLE_TREE) && ![hash40("special_n3"), hash40("special_n2_fail")].contains(&motion_kind) {
                            ...
                            if (VariableModule::get_float((boma) as *mut _, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLOAT_TREE_POS_X)-pos_x).abs() < X_DIST && (VariableModule::get_float((boma) as *mut _, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLOAT_TREE_POS_Y)-pos_y).abs() < Y_DIST && is_facing_tree  {
                    ...
                    if situation_kind != SITUATION_KIND_GROUND && motion_kind != hash40("special_air_n") {
                        ...
                        VariableModule::set_flag((boma) as *mut _, true, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_HAS_BEEN_AIR);
                    }
                    if [hash40("special_n3")].contains(&motion_kind) {
                        if (!ArticleModule::is_exist(fighter.module_accessor, *FIGHTER_MURABITO_GENERATE_ARTICLE_TREE) || VariableModule::is_flag((boma) as *mut _, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_IS_FALLEN) ) {
...
unsafe extern "C" fn tree_frame(weapon: &mut L2CFighterBase) {
    unsafe {
        ...
        if status_kind == *WEAPON_MURABITO_TREE_STATUS_KIND_STAND || status_kind == *WEAPON_MURABITO_TREE_STATUS_KIND_APPEAR {
            VariableModule::set_flag((boma) as *mut _, false, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_IS_FALLEN);
        } else {
            VariableModule::set_flag((boma) as *mut _, true, FIGHTER_MURABITO_INSTANCE_WORK_ID_FLAG_IS_FALLEN);
        }
        VariableModule::set_float((boma) as *mut _, PostureModule::pos_x(weapon.module_accessor), FIGHTER_MURABITO_INSTANCE_WORK_ID_FLOAT_TREE_POS_X);
        VariableModule::set_float((boma) as *mut _, PostureModule::pos_y(weapon.module_accessor), FIGHTER_MURABITO_INSTANCE_WORK_ID_FLOAT_TREE_POS_Y);
    }
}
