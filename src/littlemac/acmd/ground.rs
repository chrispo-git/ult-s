...
unsafe extern "C" fn mac_jab1_eff(agent: &mut L2CAgentBase) {
	let color_id = WorkModule::get_int(agent.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_COLOR) - get_lowest_marked_costume("littlemac","littlemac") as i32;
    frame(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
        macros::EFFECT(agent, Hash40::new("littlemac_attack_line_glove"), Hash40::new("top"), -1, 10.5, -1.5, 0, 0, 0, 0.75, 0, 0, 0, 0, 0, 0, true);
		match color_id {
			1 => macros::LAST_PARTICLE_SET_COLOR(agent, 1, 0.6, 0.3),
			2 => macros::LAST_PARTICLE_SET_COLOR(agent, 0.4, 0.4, 0.4),
			3 => macros::LAST_PARTICLE_SET_COLOR(agent, 1, 0.3, 0.3),
			7 => macros::LAST_PARTICLE_SET_COLOR(agent, 1, 0.4, 0.5),
			_ => macros::LAST_PARTICLE_SET_COLOR(agent, 0.43, 1, 0.3),
		};
    }
    frame(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
