    ...
    if os.path.exists(r'aarch64-skyline-switch'):
        ...
        if os.path.exists(r'release'):
            ...
            if os.path.exists(r'romfs'):
                print("Starting copy")
                copytree(r'romfs/fighter/common', r'releases/ultimate/mods/Ultimate S Arcropolis/fighter/common')
                os.remove(r'releases/ultimate/mods/Ultimate S Arcropolis/fighter/common/param/fighter_param.prcxml')
                copytree(r'romfs/effect', r'releases/ultimate/mods/Ultimate S Arcropolis/effect')
                copytree(r'romfs/ui', r'releases/ultimate/mods/Ultimate S Arcropolis/ui')
                copytree(r'romfs/camera', r'releases/ultimate/mods/Ultimate S Arcropolis/camera')
                copytree(r'romfs/sound', r'releases/ultimate/mods/Ultimate S Arcropolis/sound')
                copytree(r'romfs/prebuilt', r'releases/ultimate/mods/Ultimate S Arcropolis/prebuilt')
                copytree(r'romfs/stream;', r'releases/ultimate/mods/Ultimate S Arcropolis/stream;')
                shutil.copy(r'romfs/config_param.toml', r'releases/ultimate/mods/Ultimate S Arcropolis/config_param.toml')
                f = open(r'releases/ultimate/mods/Ultimate S Arcropolis/config_param.toml')
                config_param = f.readlines()
                f.close()
                cut = config_param.index("# CUSTOM CHARACTERS\n")
                cut_config_param = config_param[cut:]
                f = open(r'releases/ultimate/mods/Ultimate S Arcropolis/config_param.toml', "w")
                for i in cut_config_param:
                    f.write(i)
                f.close()
                shutil.copy(r'romfs/config.json', r'releases/ultimate/mods/Ultimate S Arcropolis/config.json')
                shutil.copy(r'romfs/victory.toml', r'releases/ultimate/mods/Ultimate S Arcropolis/victory.toml')
                for i in os.listdir(r'romfs/fighter'):
                    for root, dirs, files in os.walk(f"romfs/fighter/{i}/", topdown=False):
                        for name in dirs:
                            if "c1" in name:
                                path = os.path.join(root, name)
                                new_path = path.replace("romfs", "releases/ultimate/mods/Ultimate S Arcropolis")
