...
def run_lite():
    ...
    f = open("resources/config_param.toml")
    fighter_param = f.readlines()
    f.close()
        ...
        if "#" in fighter_param[i]:
            ...
            for f in excluded:
                if f == "element":
                    ...
                    if f"PIT" in fighter_param[i] and f"PITB" not in fighter_param[i]:
                        deleting = True
                        to_continue = True
                        fighter_param[i] = f''
                        continue
                    ...
                    if (
                        f"# {f.upper()}" in fighter_param[i] 
                        or f"#{f.upper()}" in fighter_param[i]
                        or f"/ {f.upper()}" in fighter_param[i]
                        or f"# {f.upper()} /" in fighter_param[i]
                        or f"#{f.upper()} /" in fighter_param[i]
                    ):
    ...
    f = open("ultimate/mods/Ultimate S Arcropolis/config_param.toml", "w")
    for i in fighter_param:
        f.write(i)
    f.close()
...
def grab_dependencies():
    ...
    #OSE
    response = requests.get("https://api.github.com/repos/blu-dev/one-slot-effects/releases/latest")
    latest_ver = response.json()["tag_name"]
    download_dependency = f"https://github.com/blu-dev/one-slot-effects/releases/download/{latest_ver}/libone_slot_eff.nro"
    r = requests.get(download_dependency)
    f = open("downloads/romfs/plugins/libone_slot_eff.nro","wb")
    f.write(r.content)
    f.close()
...
if not os.path.exists("resources/fighter_param.prcxml"):
        ...
        for i in lines:
            f.write(i)
        f.close()
if not os.path.exists("resources/config_param.toml"):
        shutil.copy("ultimate/mods/Ultimate S Arcropolis/config_param.toml","resources/config_param.toml")
